...
class ContextualHumanoid(Humanoid):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.all_system_params.update({f"damping_{i}": i + len(self.all_system_params) for i in range(21)})
        ...
        for env_id in env_ids:
            ...
            for param_name, param_idx in system_params.items():
                ...
                if attr_name in self.rigid_body_params:
                    for idx in range(attr_idx*3 + 4, attr_idx*3 + 4 + 3):
